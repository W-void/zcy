...
from model import UNet, UNetWithAttention, UNetWithAttAndAspp, conv1, UNetWithGroupConv, SpoonNet, rendNet
...
def train(epo_num=10):
    ...
    # net_pretrained = torch.load("./checkpoints_attention/SpoonNet_0.pt")
    net = rendNet(n_channels=10, n_classes=2)
    modelName = 'rendNet'
    # net = UNet(10, 2)
    ...
    for epo in range(epo_num):
        ...
        for index, (_, bag, bag_msk) in enumerate(train_dataloader):
            ...
            # loss = 0.2 * criterion(output, bag_msk) + 1 * criterion(spectral, bag_msk)
